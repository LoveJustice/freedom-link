...
from freedomlink.models import InterceptionRecord
from freedomlink.models import Story
from freedomlink.models import StoryDelivery
from freedomlink.models import Donor
...
irf_data = [
    ...
    # Red Flags
    BooleanCsvField("wife_under_18", "3.2 Wife is under 18"),
    ...
    BooleanCsvField("noticed_hesitant", "Noticed they were hesitant", "7.2 Seemed Hesitant"),
    BooleanCsvField("noticed_nervous_or_afraid", "7.3 Was nervous or afraid"),
    BooleanCsvField("noticed_hurrying", "7.4 Was hurrying"),
    BooleanCsvField("noticed_drugged_or_drowsy", "7.5 Seemed Drugged or Drowsy"),
    
    BooleanCsvField("noticed_new_clothes", "7.6 They Were Wearing New Clothes"),
    BooleanCsvField("noticed_dirty_clothes", "7.7 They Had Dirty Clothes"),
    BooleanCsvField("noticed_carrying_full_bags", "7.8 They Were Carrying Full Bags"),
    BooleanCsvField("noticed_village_dress", "7.9 They Were Wearing Village Dress"),
    
    BooleanCsvField("noticed_indian_looking", "7.1 That They Looked Indian"),
    BooleanCsvField("noticed_typical_village_look", "7.11 They Had A Typical Village Look"),
    BooleanCsvField("noticed_looked_like_agent", "7.12 They Looked Like An Agent"),
    BooleanCsvField("noticed_caste_difference", "7.13 Their Caste Was Different"),
    BooleanCsvField("noticed_young_looking", "7.14 That They Looked Young"),
    
    BooleanCsvField("noticed_waiting_sitting", "7.15 That They Were Sitting/Waiting"),
    BooleanCsvField("noticed_walking_to_border", "7.16 They Were Walking To The Border"),
    BooleanCsvField("noticed_roaming_around", "7.17 They Were Roaming Around"),
    BooleanCsvField("noticed_exiting_vehicle", "7.18 Them Exiting A Vehicle"),
    BooleanCsvField("noticed_heading_to_vehicle", "7.19 Them Heading Into A Vehicle"),
    BooleanCsvField("noticed_in_a_vehicle", "7.2 Them In A Vehicle"),
    BooleanCsvField("noticed_in_a_rickshaw", "7.21 Them In A Rickshaw"),
    BooleanCsvField("noticed_in_a_cart", "7.22 Them In A Cart"),
    BooleanCsvField("noticed_carrying_a_baby", "7.23 Them Carrying A Baby"),
...
def import_story_row(storyDict):
    ...
    # If IRF has already been created, linke this story to it
    irf_nbr = storyDict[spreadsheet_header_from_export_header(irf_data[0].title)]
    '''
    try:
        irf = InterceptionRecord.get(irf_number=irf_nbr)
    except:
